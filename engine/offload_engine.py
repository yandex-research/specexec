...
class Offload_KV_Cache:
    def __init__(self, config: LlamaConfig, batch_size: int = 1, max_length: int = 256, device: str = "cuda:0", dtype=torch.float16) -> None:
        self.config = config
...
class Llama:
    def __init__(self, model_name: str, max_length: int = 256, device: str = "cuda:0", dtype=torch.float16) -> None:
    ...
    def layer_compute(
        self, buffer: LlamaLayerBuffer, layer_idx: int, hidden_states: torch.FloatTensor, position_ids: torch.LongTensor, attention_mask: torch.FloatTensor
    ):
    ...
    def inference(self, input_ids: torch.LongTensor, position_ids: torch.LongTensor, attention_mask: torch.FloatTensor):
...
class OffloadEngine:
    def __init__(self, max_length: int, model_name_or_path: str, dtype=torch.float16, device="cuda:0") -> None:
...
if __name__ == "__main__":
    ...
    input_ids = torch.LongTensor([[1, 21429, 29899, 6451, 22545, 1078, 505]]).cuda()
    position_ids = torch.LongTensor([[0, 1, 2, 3, 4, 5, 6]]).cuda()
