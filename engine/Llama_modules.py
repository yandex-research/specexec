...
from transformers.models.llama.modeling_llama import ACT2FN  # noqa: F401
from transformers.models.llama.modeling_llama import LlamaConfig  # , LlamaMLP, LlamaRMSNorm, LlamaRotaryEmbedding, repeat_kv, apply_rotary_pos_emb
...
class LlamaAttention_FI(nn.Module):
    ...
    def forward(
        ...
        # cos, sin = self.rotary_emb(value_states.dtype, seq_len=key_states.shape[-2])  # max_length)
        ...
        attn_output = F.scaled_dot_product_attention(query_states, key_states, value_states, attn_mask=attention_mask, dropout_p=0.0, is_causal=False)
...
class LlamaDecoderLayer_FI(nn.Module):
    attn_class = LlamaAttention_FI

    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = self.attn_class(config=config, layer_idx=layer_idx)
...
class LlamaDecoderLayer_TG(LlamaDecoderLayer_FI):
    attn_class = LlamaAttention_TG
